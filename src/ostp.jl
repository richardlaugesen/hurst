...
See also:
[`ostp_params_to_array(pars)`](@ref),
[`ostp_params_range()`](@ref),
[`ostp_run_step(rain, pet, state, pars)`](@ref)
"""
...
with keys defined by the `ostp_params_from_array(arr)` function.
...
See also:
[`ostp_params_from_array(arr)`](@ref),
[`ostp_params_range()`](@ref)
"""
...
keys defined by the `ostp_params_from_array(arr)` function and ready to be used
...
This set of random parameters has keys defined by the `ostp_params_from_array(arr)`
...
The `pars` argument should have keys defined by the `ostp_params_from_array(arr)`
function.
...
The `pars` argument should have keys defined by the `ostp_params_from_array(arr)`
function and the `state` with keys defined by `ostp_init_state(pars)`.
